...
WP-001, WP-101, WP-102 concluídos (ver [relatório WP-102](wp-102-persistence-report.md)). **WP-103/104/105 concluídos** — os 10 endpoints do OpenAPI implementados e testados. **WP-301 concluído** — agente Android com cliente HTTP real (`core/network` + `data/RemoteActivationRepository`) atrás da flag `USE_REAL_API`. **WP-201 concluído** — API de conta (`account/me`, `GET /devices`, `activation/codes`) + **console web `apps/web-admin`** (Next.js, porta 3001; backend na 3000). Backend: 19 unit + 26 integração verdes. Web: `npm install` + `npm run build` verdes. Próximos: prova e2e ao vivo (backend+web+emulador), hardening de tokens (Keystore), trilha Business (docs 34/35). Não encadear execução sem autorização.
