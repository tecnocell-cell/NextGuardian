...
export function evaluateRisk(device: DevicePosture, policy: PolicyThresholds, now: Date): RiskResult {
  ...
  if (device.pairingState === 'REVOKED') {
    reasons.push({ code: 'ENROLLMENT_REVOKED', label: 'Vínculo revogado', weight: 50 });
  } else if (device.pairingState !== 'PAIRED') {
    reasons.push({ code: 'NOT_PAIRED', label: 'Dispositivo não vinculado', weight: 40 });
  }

  let neverSeen = false;
  if (device.lastSyncAt === null) {
    neverSeen = true;
    reasons.push({ code: 'NEVER_SEEN', label: 'Sem primeira sincronização', weight: 10 });
  } else {
    const hours = (now.getTime() - device.lastSyncAt.getTime()) / HOUR;
    if (hours > policy.maxOfflineHours) {
      reasons.push({ code: 'OFFLINE_TOO_LONG', label: `Sem comunicação há mais de ${policy.maxOfflineHours}h`, weight: 25 });
    }
  }

  if (compareVersions(device.appVersion, policy.minAppVersion) < 0) {
    reasons.push({ code: 'AGENT_OUTDATED', label: `Agente abaixo de ${policy.minAppVersion}`, weight: 15 });
  }
